...
function randomizePortals(locations) {
  ...
  for (let i = 0; i < directions.length; i++) {
    ...
    locations[shuffled[i]].exits[reverseDirection(directions[i])] =
      "Central Hub";
  }
...
const initialLocations = {
  "Central Hub": {
    description:
      "A mystical nexus where time flows normally. Portals shimmer in all directions.",
    welcome:
      "🌟 Welcome to the Central Hub! Choose your time adventure wisely.",
    key: null,
    exits: {},
    timeEffect: "normal",
    hasQuestion: false,
  ...
  "Bermuda Triangle": {
    description:
      "A mysterious triangular vortex where time accelerates dramatically. Reality bends around you.",
    welcome:
      "🔺 Entering Bermuda Triangle! Time is speeding up - move quickly!",
    key: "Triangle Key",
    exits: {},
  ...
  Stonehenge: {
    description:
      "Ancient stone circles where time moves sluggishly, as if weighted by millennia.",
    welcome:
      "🗿 Welcome to Stonehenge! Time drags heavily here - use it wisely.",
    key: "Stone Key",
    exits: {},
  ...
  "Crooked Forest": {
    description:
      "A twisted woodland where time flows in reverse, undoing moments as they pass.",
    welcome:
      "🌲 Entering Crooked Forest! Time flows backward - reality unravels!",
    key: "Forest Key",
    exits: {},
    timeEffect: "reverse",
    hasQuestion: true,
  ...
  "Treasure Vault": {
    description:
      "The legendary treasure vault, accessible only to those who have mastered time itself!",
...
const getInitialGameState = (locations) => {
  ...
  if (saved) {
    ...
    if (
      ...
      currentLocation !== "Treasure Vault"
    ) {
      parsed.timeEffectStart =
        now - (parsed.locationTimer > LOCATION_TIME_LIMIT - 10 ? 0 : 10000);
    }
    // --- END MODIFICATION ---
    if (
      currentLocation !== "Central Hub" &&
      ...
      const effectActive =
        parsed.timeEffectStart && now - parsed.timeEffectStart <= 10000;
      if (effectActive) {
        switch (timeEffect) {
          case "reverse":
...
function App() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setGameState((prev) => {
        ...
        if (
          ...
          effectActive =
            prev.timeEffectStart && now - prev.timeEffectStart <= 10000;
          ...
          if (effectActive) {
            switch (currentLocation.timeEffect) {
              ...
              case "decelerated":
                gameTimeDelta =
                  deltaTime * (currentLocation.timeModifier || 0.5);
                break;
              default:
                gameTimeDelta = deltaTime;
            }
    ...
    // Dependency on locations[gameState.location] can cause infinite loop, so don't include locations
    // eslint-disable-next-line
  }, [gameState.gameActive, gameState.location]);
  ...
  useEffect(() => {
    ...
    log("");
    log(
      "🎯 YOUR MISSION: Collect all 3 keys from different time-distorted locations!",
      "treasure"
    );
    log(
      "⏰ WARNING: Each location has unique time effects that will challenge you!",
      "warning"
    );
    log("");
    log("💡 TIPS:", "success");
    ...
    log(
      "• Watch your location timer - you have 2 minutes per special location!"
    );
  ...
  function enterLocation(loc = null) {
    ...
    if (exits.length > 0) {
      ...
      if (exits.includes("treasure")) {
        if (checkTreasureUnlock()) {
          log(
            '💰 ✨ TREASURE VAULT UNLOCKED! Type "treasure" to enter! ✨',
            "treasure"
          );
        } else {
          const keysNeeded = 3 - gameState.keys.length;
          log(
            `🔐 Treasure Vault: LOCKED (Need ${keysNeeded} more keys)`,
            "warning"
          );
        }
      }
    ...
    if (currentLoc.key && !gameState.keys.includes(currentLoc.key)) {
      log(
        `🗝 You see a ${currentLoc.key} glinting nearby! Type 'collect' to take it.`
      );
    }

    if (
      loc === "Treasure Vault" ||
      (!loc && gameState.location === "Treasure Vault")
    ) {
      setGameState((prev) => ({ ...prev, score: prev.score + 10000 }));
      log("🎉 CONGRATULATIONS! YOU WON! 🎉", "treasure");
      log(
        `🏆 VICTORY! FINAL SCORE: ${(gameState.score + 10000).toLocaleString()} POINTS!`,
        "treasure"
      );
  ...
  function showFinalStatus() {
    ...
    log(
      `🗝 Keys Collected: ${gameState.keys.join(", ") || "None"} (${
        gameState.keys.length
      }/3)`
    );
    ...
    log(
      `🗝 Keys Collected: ${gameState.keys.length} × 1,000 = ${(
        gameState.keys.length * 1000
      ).toLocaleString()} pts`
    );
    ...
    log(
      `🤔 Questions Answered: ${questionsAnswered} × 500 = ${(
    ...
    switch (command) {
      ...
      case "treasure": {
        ...
        if (!currentLocation.exits[command]) {
          ...
          const keysNeeded = 3 - gameState.keys.length;
          log(
            `🔐 The treasure vault is sealed! You need ${keysNeeded} more keys.`,
            "error"
          );
          log(
            `🗝 Current keys: ${gameState.keys.join(", ") || "None"} (${
              gameState.keys.length
            }/3)`,
            "warning"
          );
          ...
          if (
            currentLocation.hasQuestion &&
            !currentLocation.questionAnswered
          ) {
            if (!askQuestion()) {
      ...
      case "collect": {
        ...
        if (locationObj.key && !gameState.keys.includes(locationObj.key)) {
          ...
          if (checkTreasureUnlock()) {
            ...
            log(
              "💰 The treasure vault is now accessible from Central Hub!",
              "treasure"
            );
          } else {
            ...
            log(
              `🗝 ${remaining} more key${
                remaining > 1 ? "s" : ""
              } needed for the treasure vault!`,
              "success"
            );
          }
        } else {
          log(
            "❌ There's no key to collect here or you already have it.",
            "error"
          );
        }
        break;
      ...
      case "answer": {
        ...
        if (
          args &&
          args.toLowerCase().includes(currentLoc.answer.toLowerCase())
        ) {
    ...
    <div>
      ...
      <div id="status-panel">
        ...
        <div className="status-item">
          ...
          <span
            className={`status-value${
              gameState.locationTimer <= 30 && gameState.locationTimer > 0
                ? " pulsing"
                : ""
            }`}
            style={{
          ...
          >
            {gameState.location !== "Central Hub" &&
            gameState.location !== "Treasure Vault"
        ...
        <div className="status-item">
          ...
          <span className="status-value time-effect">
            {locations[gameState.location]?.timeEffect?.toUpperCase() ||
              "NORMAL"}
          </span>
        </div>
        <div className="status-item">
      ...
      <form className="input-area-wrap" onSubmit={handleCommand}>
        ...
        <div style={{ marginTop: 10, textAlign: "left" }}>
          <button id="reset-button" type="button" onClick={handleResetClick}>
            🔄 Reset Game
          </button>
        </div>
      </form>
